...
export function printCalendar({ currentDate, events, title, primaryColor = "#4f46e5", orgName }: PrintOptions) {
  ...
  const eventsByDate = new Map<string, PrintEvent[]>();
  ...
  const tableRows = weeks.map((week) => {
    const cells = week.map((day) => {
      ...
      const eventsHtml = dayEvents.map((ev) => {
        ...
        const dotColor = ev.categories?.[0]?.color || primaryColor;
        return `<div class="ev"><span class="dot" style="background:${esc(dotColor)}"></span><div class="ev-body"><div class="ev-t">${esc(ev.title)}</div><div class="ev-m">${esc(time)}${loc ? ` &middot; ${esc(loc)}` : ""}</div></div></div>`;
      }).join("");
      return `<td class="dc${!inMonth ? " out" : ""}"><div class="dn">${format(day, "d")}</div>${eventsHtml}</td>`;
    }).join("");
    return `<tr>${cells}</tr>`;
  }).join("");
  ...
  const sidebarHtml = monthEvents.map((ev) => {
    ...
    const dotColor = ev.categories?.[0]?.color || primaryColor;
    return `<div class="se"><div class="sd" style="background:${esc(dotColor)}"></div><div><div class="st">${esc(ev.title)}</div><div class="sm">${esc(date)} &middot; ${esc(time)}</div>${loc ? `<div class="sm">${esc(loc)}</div>` : ""}<div class="sm">${esc(cost)}</div></div></div>`;
  }).join("");
...
<head>
<meta charset="utf-8">
<title>${esc(title || "Community Calendar")} – ${esc(monthLabel)}</title>
<style>
...
.hdr .org{font-size:11px;color:#888;font-weight:600}
...
.ev-body{min-width:0;overflow:hidden}
.ev-t{font-size:7.5px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.ev-m{font-size:6.5px;color:#888;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<div class="hdr">
  <div>
    <h1>${esc(title || "Community Calendar")}</h1>
