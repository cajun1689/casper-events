...
import { sponsorsApi, type EventSponsor } from "@/lib/api";
...
export function EventSponsorsSection({ eventId }: EventSponsorsSectionProps) {
  ...
  const [form, setForm] = useState({
    ...
    websiteUrl: "" as string | null,
    level: "community" as string,
  });

  useEffect(() => {
    sponsorsApi.list(eventId).then((res) => {
  ...
  async function handleAdd() {
    ...
    try {
      const created = await sponsorsApi.create(eventId, {
        name: form.name.trim(),
  ...
  async function handleUpdate(id: string) {
    if (!form.name.trim()) return;
    try {
      const updated = await sponsorsApi.update(eventId, id, {
        name: form.name.trim(),
        logoUrl: form.logoUrl || null,
        websiteUrl: form.websiteUrl || null,
        level: form.level,
  ...
  async function handleDelete(id: string) {
    try {
      await sponsorsApi.delete(eventId, id);
  ...
  function startEdit(sponsor: EventSponsor) {
    setForm({
      name: sponsor.name,
  ...
  return (
    <section className="rounded-2xl border border-gray-200/60 bg-white/80 p-6 shadow-sm backdrop-blur-sm space-y-5">
      ...
      <p className="text-xs text-gray-400 -mt-3">Add sponsor logos and links for the poster board embed view.</p>
